...
def get_feedback(score, matched, missing, role):
    ...
    if score < 0.4:
        return (
            f"❌ Low relevance to **{role}**. "
            f"Consider adding: {', '.join(missing[:5])}."
        )

    elif score < 0.7:
        return (
            f"⚠️ Partial match for **{role}**. "
            f"Improve by highlighting: {', '.join(missing[:3])}."
        )

    else:
        return (
            f"✅ Strong fit for **{role}** with skills: "
            f"{', '.join(matched[:5])}."
        )
...
st.set_page_config(
    page_title="Resume Analyzer - Bulk ZIP Upload",
    layout="centered"
)
...
st.write(
    ...
    "and get matching scores by role!"
)
...
desired_role = st.text_input(
    "🎯 Enter Desired Role (e.g., Data Scientist)"
)
...
if zip_file and desired_role:
    ...
    if desired_role not in job_roles:
        ...
        st.error(
            "❗ Role not found. Please choose a valid predefined role."
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if not pdf_files:
                ...
                st.warning(
                    "No PDF files found in the uploaded ZIP."
                )

            else:

                st.subheader(
                    f"📊 Resume Matching for Role: {desired_role}"
                )
                ...
                for res in results:
                    ...
                    st.write(
                        f"**Match Score:** "
                        f"{round(res['score'] * 100, 2)}%"
                    )

                    st.write(
                        f"**Resume Grade:** "
                        f"{calculate_grade(res['score'])}"
                    )

                    st.write(
                        f"✅ **Matched Skills:** "
                        f"{', '.join(res['matched_keywords'][:7]) or 'None'}"
                    )

                    st.write(
                        f"❌ **Missing Skills:** "
                        f"{', '.join(res['missing_keywords'][:7]) or 'None'}"
                    )
    ...
    st.info(
        "Please upload a .zip file and enter a valid desired role."
    )